            ...
            <div className="container mx-auto px-6">
                ...
                <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
                    {steps.map((step, index) => (
                        <Card key={index} className="bg-white/5 border border-white/10 text-center p-8 rounded-2xl transform transition-transform duration-300 hover:-translate-y-2">
                            <CardHeader className="p-0">
                                {step.icon}
                                <CardTitle className="font-headline text-2xl text-white">{step.title}</CardTitle>
                                <CardDescription className="font-body text-white/70 leading-relaxed mt-2">{step.description}</CardDescription>
                            </CardHeader>
                        </Card>
                    ))}
                </div>
